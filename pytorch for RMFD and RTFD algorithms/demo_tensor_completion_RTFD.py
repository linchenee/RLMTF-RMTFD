...
parser.add_argument('--lambda1', type=float, default=1, help='regularization parameter')
parser.add_argument('--lambda2', type=float, default=400, help='regularization parameter')
...
num_image = 1  # number of images
n1, n2, n3 = 256, 256, 31  # size of the image
rank1 = 200  # tensor multi-rank = [rank1, rank2*ones(1, n3-1)]
rank2 = int(rank1 / 3)
pad = 'reflection'
test_mat = sio.loadmat('./%s/%s' % (args.data_dir, 'CAVE_sampling_rate_10.mat'))  # sampling rate=10%
...
def RTFD(Y, img_id, img_gt, M, M_comp, mu):
    ...
    Xd = dct.dct(torch.mul(torch.ones(n1, n2, n3).to(device), M_comp) + torch.mul(Y, M), norm='ortho')
    temp = Xd
    ...
    for j in range(num_iter):
        ...
        for i in range(inner_iter):
            ...
            total_loss = norm2_loss(net_output - B) * mu * 0.5 + lambda2 * norm2_loss(torch.mul(M_4D, net_output - Y_4D))
            total_loss.backward()
            optimizer.step()
...
torch.manual_seed(3)
rec_PSNR = np.zeros((num_image, 1))
rec_img = np.zeros((num_image, n1, n2, n3))
for i in range(num_image):
    data0 = (torch.from_numpy(test_data[i, 0:n1*n2*n3]).type(torch.FloatTensor))
    data1 = (torch.from_numpy(test_data[i, n1*n2*n3:n1*n2*n3*2]).type(torch.FloatTensor))
    ...
    img_input = data1.view(n1, n2, n3).to(device)  # incomplete image
    M = data2.view(n1, n2, n3).to(device)  # binary mask that indicates the locations of missing and known entries in the tensor
...
sio.savemat('Result_CAVE_sampling_rate_10.mat', {'Result': rec_img})
print(rec_PSNR)
