...
parser.add_argument('--show_iter', type=int, default=200, help='show the loss during the iteration')
...
num_img = 20  # number of images
n1, n2, n3 = 321, 481, 3  # size of the image
rank1 = 200  # tensor multi-rank = [rank1, rank2*ones(1, n3-1)]
rank2 = int(rank1 / 5)
pad = 'reflection'
test_mat = sio.loadmat('./%s/%s' % (args.data_dir, 'BSD_salt_and_ pepper_noise_10dB.mat'))
...
def RTFD(Y, img_id, img_gt, M, M_comp, mu):
    ...
    for j in range(num_iter):
        ...
        #     break
        if j % show_iter == 0:
            rec_PSNR = psnr(np.clip(X.detach().cpu().numpy() * 255, 0, 255), img_gt * 255, M_comp.cpu().numpy())
    ...
    rec_img = np.clip(X.detach().cpu().numpy() * 255, 0, 255)
...
for i in range(num_img):
    ...
    img_noisy = data1.view(n1, n2, n3).to(device)  # noisy image
...
# sio.savemat('Result_BSD_salt_and_ pepper_noise_10dB.mat', {'Result': rec_img})
print(rec_PSNR)
